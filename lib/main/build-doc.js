...
const _path = path.join(basePath, 'lodash.js')
...
const config = {
	...
	site: {
		...
		sourceLink: '<a href="${sourceHref}" class="md-link">${sourceHref}</a>',
		tocHref: '',
		tocLink: '',
		sublinks: npmLink()
	}
...
function npmLink() {
	return (
		'<% if (name == "templateSettings" || !/^(?:methods|properties|seq)$/i.test(category)) { print("https://github.com/flavioespinoza/datedash/blob/master/methods/" + name.toLowerCase() + ".js' +
		'");} %>'
	)
}
