...
function tidyHighlights($) {
	$('.highlight').each(function() {
		...
		// 	const attr = _(classes)
		// 		.intersection(highlights[ext])
		// 		.join(' ')
...
function build() {
	const markdown = fs
		...
		.replace(
			...
			(match, href1, href2) => `<p class="pee-balls"><a class="a-balls" href="${href1}">source</a><a href="${href2}">npm package</a></p>`
		)
	...
	fs.writeFile(path.join(docPath, 'doc' + '.html'), html_2, util.pitch)
}
