...
function build() {
	const markdown = fs
		...
		.replace(
			...
			(match, href1, href2) => `<p><a href="${href1}">source</a> <a href="${href2}">npm package</a></p>`
		)
	...
	const html = [
		'<!DOCTYPE html>',
		'<html class="docs gr__lodash_com" style="overflow: auto;">',
		'',
		'<head>',
		'<meta charset="utf-8">',
	...
	log.lightYellow(path)

	fs.writeFile(path.join(docPath, 'docs' + '.html'), html, util.pitch)
}
